  ...
  then
    ...
    then
      ...
      then
        ...
        # send to main menu
        MAIN_MENU "I have put you down for the $BIKE_INFO_FORMATTED Bike, $(echo $CUSTOMER_NAME | sed -E 's/^ *| *$//g')." # trim all white spaces from font and end of the text
      fi
    fi
  fi
}
...
RETURN_MENU() {
  ...
  if [[ ! $BIKE_ID_TO_RETURN =~ ^[0-9]+$ ]]
    then
      ...
      RENTAL_ID=$($PSQL "SELECT rental_id FROM rentals INNER JOIN customers USING(customer_id) WHERE phone = '$PHONE_NUMBER' AND bike_id = $BIKE_ID_TO_RETURN
